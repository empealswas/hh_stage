...
`
export default function PupilsSearchList() {
    ...
    const addPupilsToClassroom = async () => {
        ...
        setLoading(true);
        selectedPupils?.forEach(async (pupil) => {
            const input = {
                classroomID: classroomId,
                pupilID: pupil.id
            };
            await API.graphql(graphqlOperation(createPupilClassroom, {input}));
        })
        setLoading(false);
    }
    const disabled = selectedPupils?.length == 0;
    return (
