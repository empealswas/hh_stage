...
export default function AnalyticsSleepChart() {
        ...
        const getAverage = async () => {
            if (user instanceof Teacher) {
                ...
                const dataToChart = results.map((value, index) => {
                    const steps = value.data?.map((item: any) => item.value /60/60 ?? 0) ?? [];
                    return {
    ...
    return (
        <Card>
            <CardHeader title="Average sleep hours for your classrooms" subheader="For the past 7 days" />
